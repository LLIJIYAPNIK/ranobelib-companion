...
// in place. On any terminal state (done/error/needs_translation/cancelled - PR 226) it
// reloads the page once rather than duplicating that state's markup here - the
// server-rendered page already knows how to show each of those, including the explicit
// "Скачать файл" link for "done" (see app/templates/download_status.html). That link
// requiring a real click (rather than this script auto-navigating to it) is what makes it
// safe for app/api/downloads.py to delete the exported file once it's been served - see
// PR 50. The script itself is only ever included for a job that's still in progress (see
...
(() => {
  ...
  const TERMINAL_STATUSES = new Set(["done", "error", "needs_translation", "cancelled"]);
