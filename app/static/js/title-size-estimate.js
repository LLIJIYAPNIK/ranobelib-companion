...
// show_title() on it.
//
// PR 203: this element now only exists once title-content-load.js has fetched and
// injected the title page's real content (GET /titles/{slug}/data) - self-running at
// parse time would find nothing yet, so this exposes an init function that call site runs
// explicitly once that markup is in the DOM.
(() => {
  function initTitleSizeEstimate() {
  ...
  window.initTitleSizeEstimate = initTitleSizeEstimate;
})();
