    ...
    *,
    ...
    try:
        async with open_client(job.slug_url) as lib:
            # download_title() rides out the API's own rate limiting internally (retrying
            # a rate-limited chapter fetch up to its max_rate_limit_retries, default 6, on
            # top of ApiClient's own per-request retries) - see ranobelib-python-sdk#41.
            # It's only actually interrupted (DownloadTitleInterruptedError, below) once
            # that budget is exhausted or something else unrecoverable happens.
        ...
        # job.completed/job.total already reflect exc.completed/exc.total - on_chapter()
        # was called for each of them before the underlying error hit. Map the *cause*
        # (what actually stopped the download) for the user-facing message, not this
        # wrapper - it isn't itself in the CLAUDE.md error table.
        cause = exc.__cause__
        logger.warning(
