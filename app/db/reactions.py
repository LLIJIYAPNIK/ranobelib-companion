...
``''`` rather than NULL for "no branch selected" - SQLite's UNIQUE constraint treats NULLs
as distinct from each other, which would silently defeat "one reaction per user per
paragraph" for chapters with no branch_id.
...
def toggle_reaction(
    ...
    existing = conn.execute(
        ...
        "WHERE user_id = ? AND slug_url = ? AND volume = ? AND number = ? "
        "AND branch_id = ? AND paragraph_index = ?",
...
def user_reactions(
    ...
    rows = conn.execute(
        ...
        "WHERE user_id = ? AND slug_url = ? AND volume = ? AND number = ? AND branch_id = ?",
