...
async def toggle_comment_reaction(
    conn: AsyncConnection, user_id: int, comment_id: int, value: int
) -> int | None:
    ...
    cursor = await conn.execute("SELECT 1 FROM comments WHERE id = %s", (comment_id,))
...
async def count_reactions_for_comment(conn: AsyncConnection, comment_id: int) -> dict[str, int]:
...
async def user_reactions_for_paragraph(
    conn: AsyncConnection,
    user_id: int,
    slug_url: str,
    volume: str,
