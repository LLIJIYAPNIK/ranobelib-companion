...
@pytest.fixture
async def user(monkeypatch: pytest.MonkeyPatch) -> Iterator[User]:
...
async def test_build_activity_summary_counts_and_enriches_chapters_read_today(
    ...
    async with connection() as conn:
        await record_chapter_read(conn, user.id, "6712--test-novel", "1", "5")
        await record_chapter_read(conn, user.id, "6712--test-novel", "1", "6")
...
async def test_build_activity_summary_falls_back_when_title_unreachable(user: User) -> None:
    async with connection() as conn:
        await record_chapter_read(conn, user.id, "6712--gone-novel", "1", "5")
...
async def test_build_activity_summary_formats_minutes_only(user: User) -> None:
    async with connection() as conn:
        await record_heartbeat(conn, user.id, "6712--test-novel", 45 * 60)
...
async def test_build_activity_summary_formats_hours_and_minutes(user: User) -> None:
    async with connection() as conn:
        await record_heartbeat(conn, user.id, "6712--test-novel", 90 * 60)
...
async def test_build_activity_summary_includes_downloads_today(user: User) -> None:
    async with connection() as conn:
        await record_download(conn, user.id, "6712--test-novel", "epub", "done", 10, None)
